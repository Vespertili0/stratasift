...
from stratasift.eval.bootstrapper import EvaluationScores, GoldenPair, bootstrap_golden_records
    ...
    if quarantine_path.exists():
        for db_file in quarantine_path.glob("tmp-context-*.json"):
            ...
            if not db_file.exists():
                log_event(f"      ⚠️ Missing tmp-ContextDB at {db_file}, skipping evaluation.")
                continue
            ...
            try:
                ...
                log_event(f"      ⚠️ Corrupted tmp-ContextDB at {db_file}, skipping evaluation.")
                continue
            ...
            if mock_mode:
                ...
                try:
                    ...
                    log_event(f"      ⚠️ deepeval import failed: {e}. Falling back to mock scores.")
                    scores = EvaluationScores(
            ...
            if (scores.extraction_faithfulness >= config.evaluation.threshold and 
                scores.synthesis_relevance >= config.evaluation.threshold):
                
                golden_records.append(GoldenPair(
                    source_filename=source_filename,
                    reading_directive=context_snapshot.get("reading_directive", ""),
                    central_hypothesis=context_snapshot.get("central_hypothesis", ""),
